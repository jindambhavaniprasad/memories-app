...
const Post = ({ post, setCurrentId }) => {
  ...
  const Likes = () => {
    if (post.likes.length > 0) {
      if (post.likes.find((like) => like === (user && user.result ? user.result._id : user && user.profileObj ? user.profileObj.googleId : ''))) {
  ...
  return (
    <Card className={classes.card}>
      ...
      <div className={classes.overlay2}>
        {
          (user?.result?._id === post?.creator || user?.profileObj?.googleId === post?.creator) &&
      ...
      <CardActions className={classes.cardActions}>
        <Button size="small"
          color="primary"
          disabled={!((user && user.result) || (user && user.profileObj))}
